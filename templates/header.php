<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Dashboard</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">
</head>
<body>

<!-- Barra de Navegación -->
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <a class="navbar-brand" href="#">FERRECAM</a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav">
            <li class="nav-item">
                <a class="nav-link" href="javascript:top.location.href='../../sistema/ventas.php'">Ventas</a>
            </li>
            <li class="nav-item active">
                <a class="nav-link" href="javascript:top.location.href='../../sistema/pru.php'">Inventario</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="javascript:top.location.href='../../sistema/clientes.php'">Clientes</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="javascript:top.location.href='../../sistema/proovedores.php'">Proovedores</a>
            </li> 
            <li class="nav-item">
                <a class="nav-link" href="javascript:top.location.href='../../sistema/compra.php'">Compra</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="javascript:top.location.href='../../sistema/pru.php'">Informes</a>
            </li> 
            <li class="nav-item">
                <a class="nav-link" href="javascript:top.location.href='../../sistema/baseConfig.php'">Configuración</a>
            </li>
        </ul>
    </div>
</nav>

<script src="https://code.jquery.com/jquery-3.3.1.slim.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"></script>

</body>
</html>
